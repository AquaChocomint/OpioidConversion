...
class OpioidConversion {
    constructor() {
        this.opioids = new Array;
        this.iconActive = '#ff0cfb';
        this.iconInactive = '#afafaf';
        this.init();
    }
    ...
    addTable() {
        const tbody = window.document.getElementsByTagName('tbody').item(0);
        for (const opioid of this.opioids) {
            const tr = window.document.createElement('tr');
            const icon = window.document.createElement('td');
            icon.innerText = '◯';
            icon.style.color = this.iconInactive;
            tr.appendChild(icon);
            opioid.icon = icon;
            ...
            tr.appendChild(name);
            const formula = window.document.createElement('td');
            formula.innerText = opioid.formulaReadable;
            tr.appendChild(formula);
            const box = window.document.createElement('td');
    ...
    switchDisplay() {
        const prepare = window.document.getElementById('contents-prepare');
        const pstyle = prepare.style;
        pstyle.display = (pstyle.display === 'none' ? 'block' : 'none');
        const main = window.document.getElementById('contents-main');
    ...
    getWarningElement() {
        return window.document.getElementsByClassName('contents-prepare-warning').item(0);
    }
    buildFooter() {
        const footer = window.document.getElementsByClassName('footer').item(0);
        const links = [
            {
                label: 'ソースコード',
